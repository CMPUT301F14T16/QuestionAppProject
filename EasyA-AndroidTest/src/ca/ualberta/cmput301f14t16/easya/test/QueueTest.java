...
public class QueueTest extends TestCase{
	...
	Queue queue = new Queue();
    	...
    	if (queue.lastCheck == null) {
    	    assertEquals(queue.checkForInternet(),queue.haveInternetConnection());
    	    assertEquals(queue.ForceCheckInternet(),queue.haveInternetConnection());
    	}
    	
    	if (queue.haveInternetConnection() == true) {
		    ...
		    Log.d(LOG_TAG,"Timeout caught!");
    		    ...
    		    e.printStackTrace();
    		}catch (NoClassTypeSpecifiedException e) {
    		    String LOG_TAG = null;
    		    Log.d(LOG_TAG,"This is not a class type defined");
    		    return;
    		}finally {
    		    assertTrue(pendings.size() < 4);
    		}
